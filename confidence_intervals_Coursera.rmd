...
3.  What does "95% confidence" mean?
    ...
    <li>**95% of random samples of size 60 will yield confidence
    intervals that contain the true average area of houses in Ames,
    Iowa.**</li>
...
ci <- ames %>% rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
  summarise(lower = mean(area) - 2.58 * (sd(area) / sqrt(n)), upper = mean(area) + 2.58 * (sd(area) / sqrt(n)))


ggplot(data = ci_data, aes(x = ci_bounds, y = ci_id, group = ci_id, color = capture_mu)) + geom_point(size = 2) + geom_line() + geom_vline(xintercept = params$mu, color = "darkgray")
